"""Modul pembuatan bukti guru PNG - Bolt.now / PSU"""
import random
from datetime import datetime


def generate_psu_id():
    """Generate PSU ID acak (9 digit)"""
...
def generate_psu_email(first_name, last_name):
    """
    Generate email PSU
    Format: firstName.lastName + 3-4 digit @psu.edu
    """
...
def _get_browser_context():
    """Ambil atau buat konteks browser (singleton)"""
    ...
    if _browser_context is None:
        try:
            ...
            raise Exception("Playwright belum terpasang: pip install playwright && playwright install chromium")
    return _browser_context


def _html_to_png(html_content: str, width: int = 1200, height: int = None) -> bytes:
    """Ubah HTML menjadi screenshot PNG (versi optimal: reuse browser)"""
    try:
        context = _get_browser_context()
        page = context.new_page()

        try:
            # Set konten HTML langsung, gunakan domcontentloaded (lebih cepat)
            page.set_content(html_content, wait_until='domcontentloaded')

            # Tunggu gambar selesai load (jika ada gambar eksternal)
...
def _html_to_png_batch(html_list: list[tuple[str, int, int]]) -> list[bytes]:
    """
    Generate beberapa PNG secara paralel (versi optimal)
    ...
    async def render_single(html_content: str, width: int, height: int):
        """Render satu gambar secara async"""
...
def generate_images(first_name: str, last_name: str, school_id: str = '2565'):
    """
    Generate dua PNG: kartu guru + bukti kerja (versi paralel)

    Args:
        first_name: nama depan
        last_name: nama belakang
        school_id: ID sekolah (tetap kompatibel)
    ...
    # Generate dua gambar secara paralel
    html_list = [
        (card_html, 700, 1100),
