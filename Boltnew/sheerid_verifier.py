...
class SheerIDVerifier:
    ...
    def normalize_url(url: str) -> str:
        """Normalisasi URL (biarkan apa adanya, kompatibel dengan interface lama)"""
        return url
    ...
    def create_verification(self) -> str:
        """Minta verificationId baru lewat installPageUrl"""
        body = {
        ...
        logger.info(f"✅ Berhasil mendapatkan verificationId: {self.verification_id}")
        return self.verification_id

    def _sheerid_request(
        self, method: str, url: str, body: Optional[Dict] = None
    ) -> Tuple[Dict, int]:
        """Kirim request API SheerID"""
        headers = {
    ...
    def verify(
        ...
        try:
            ...
            if not self.verification_id:
                logger.info("Meminta verificationId baru...")
                self.create_verification()

            logger.info(f"Data guru: {first_name} {last_name}")
            ...
            # Kirim data guru
            logger.info("Langkah 2/5: mengirim data guru...")
            step2_body = {
            ...
            if isinstance(step2_data, dict) and step2_data.get("currentStep") == "error":
                ...
                raise Exception(f"Error langkah 2: {error_msg}")
            ...
            # Minta URL upload dan upload dokumen
            ...
            if step4_status != 200 or not isinstance(step4_data, dict) or not step4_data.get("documents"):
                raise Exception(f"URL upload tidak ditemukan: {step4_data}")

            documents = step4_data["documents"]
            if len(documents) != len(assets):
                raise Exception("Jumlah tugas upload tidak sesuai dengan jumlah file")

            for doc, asset in zip(documents, assets):
                upload_url = doc.get("uploadUrl")
                if not upload_url:
                    raise Exception("URL upload tidak ditemukan")
                if not self._upload_to_s3(upload_url, asset["data"]):
                    raise Exception(f"Upload S3 gagal: {asset['file_name']}")
...
def main():
    ...
    print("=" * 60)
    print("Alat verifikasi guru SheerID (Python)")
    print("=" * 60)
    print()
    ...
    if len(sys.argv) > 1:
        ...
        url = input("Masukkan URL verifikasi SheerID (berisi externalUserId): ").strip()
    ...
    print(f"👉 Gunakan link: {verifier.install_page_url}")
    if verifier.verification_id:
        print(f"Berhasil mengambil verificationId: {verifier.verification_id}")
