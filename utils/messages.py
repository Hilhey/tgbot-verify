...
def get_help_message(is_admin: bool = False) -> str:
    ...
    msg = (
        ...
        f"/verify <tautan> [proxy] - Gemini One Pro (biaya {VERIFY_COST} poin)\n"
        f"/verify2 <tautan> [proxy] - ChatGPT Teacher K12 (biaya {VERIFY_COST} poin)\n"
...
def get_verify_usage_message(command: str, service_name: str) -> str:
    ...
    return (
        f"Cara penggunaan: {command} <tautan SheerID> [proxy]\n\n"
        "Contoh:\n"
        f"{command} https://services.sheerid.com/verify/xxx/?verificationId=xxx http://user:pass@host:port\n\n"
